...
export default class BlogManager {
  ...
  public async listArticlesByCondition(option, pageIndex) {
    ...
    if (option.dateRange.length && !andStatus) {
      ...
      this.sql += ` and (unix_timestamp(date_publish) between ${
        option.dateRange[0]
      } and ${option.dateRange[1]})`;
    }
  ...
  public async saveArticle(body) {
    ...
    this.sql = `insert into blg_article (title, category_id, content, text_type, date_publish)
                    values ('${body.title}', ${body.categoryId}, '${
      body.content
    }', '${body.textType}', now());`;
    return await sequelize.query(this.sql);
  }
  ...
  public async updateArticle(body) {
    this.sql =
      "update blg_article set title=:title, category_id=:category_id, content=:content where id=:id;";
    return await sequelize.query(this.sql, {
      replacements: {
        id: body.id,
        title: body.title,
  ...
  public async addCategory(body) {
    this.sql = `insert into blg_category (father_id, level,  name)
                    values (${body.fatherId}, ${body.level}, '${
      body.categoryName
    }');`;
    return await sequelize.query(this.sql);
  }
